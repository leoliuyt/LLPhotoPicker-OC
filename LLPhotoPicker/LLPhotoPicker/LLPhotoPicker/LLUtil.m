...
{
    return ((NSString *)aString).length > 0 ? aString : @"";
}

static NSString *GLOBAL_TIMEFORMAT = @"yyyy-MM-dd HH:mm:ss";

#define AppName @"艺直播"
#define HANZI_STLL 19968
...
"hzqydxlbpjllmqxqydzxsqjtzpxlcglqdcwzfhctdjjsfxjejjtlbgxsxjmyjjqpfzasyjnsydjxkjcdjsznbLLcclnjqmwnqnc"
...
+ (NSString *)createUDID
{
    //  static NSInteger magicID = 1;
    //  // LLudid 区分保证唯一性
    //  NSString* timestamp = [NSString stringWithFormat:@"LLudid%f%tu", [[NSDate date] timeIntervalSince1970], magicID++];
    //  return [timestamp MD5String];
    return [NSString stringWithFormat:@"LL-%@", [[NSUUID UUID] UUIDString]];
}

+ (char)pinyinFirstLetter:(unsigned short)aHanzi
{
    int index = aHanzi - HANZI_STLL;
...
+ (void)makeCall:(NSString *)aPhoneNumber
{
    ...
    NSString * str= [NSString stringWithFormat:@"tel:%@", aPhoneNumber];
...
+ (long)timeIntervalWith:(NSString *)timeString
{
    ...
    NSString *timeStr = [NSString stringWithFormat:@"%@:00",timeString];
    
    NSString* utcDate = [LLUtil getUTCFormateLocalDate:timeStr];
    
    
    return [LLUtil getLocalDateFormateUTCDate:utcDate];
}
...
+(long )getLocalDateFormateUTCDate:(NSString *)utcDate
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    //输入格式
    [dateFormatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ssZ"];
...
+ (BOOL)checkUseCamera{
    ...
    return [self checkUseEquipment:@"相机" support:captureDevice != nil allow:captureDeviceInput != nil camera:YES];
}
...
+ (BOOL)checkUseRecord
{
    ...
    return [self checkUseEquipment:@"麦克风" support:YES allow:bCanRecord camera:NO];
}

+ (BOOL)checkUseEquipment:(NSString *)equipment support:(BOOL)isSupport allow:(BOOL)isAllow camera:(BOOL)isCamera
{
//    LLApp *app = [[LLApp alloc] init];
//    UIViewController* vc = app.viewController;
    UIViewController *vc = nil;
    CGFloat fSystemVersion = [[[UIDevice currentDevice] systemVersion] floatValue];
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0f) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"系统版本太低，需要升级"
                                                        message:nil
                                                       delegate:nil
    ...
    if (!isSupport) {
        ...
        if (fSystemVersion < 8.0f) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSString stringWithFormat:@"你的设备不支持%@",equipment]
                                                            message:nil
                                                           delegate:nil
                                                  cancelButtonTitle:@"确定"
                                                  otherButtonTitles:nil];
            [alert show];
        } else if (fSystemVersion >= 8.0f) {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"你的设备不支持%@",equipment]
                                                                                     message:nil
    ...
    if (!isAllow) {
        ...
        if (!isCamera) {
            [self checkSystemVersion:fSystemVersion equipment:equipment vc:vc];
            
        }else if ([AVCaptureDevice respondsToSelector:@selector(authorizationStatusForMediaType:)]) {
...
+ (void)checkSystemVersion:(CGFloat)fSystemVersion equipment:(NSString *)equipment vc:(UIViewController *)vc {
    
    NSString *str = [NSString stringWithFormat:@"【%@】想访问你的%@",AppName,equipment];
    if (fSystemVersion < 8.0f) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:str
                                                        message:[NSString stringWithFormat:@"请在设备的“设置-隐私-%@”中允许访问%@",equipment,equipment]
                                                       delegate:nil
                                              cancelButtonTitle:@"确定"
                                              otherButtonTitles:nil];
        [alert show];
        ...
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"不允许" style:UIAlertActionStyleCancel handler:^(UIAlertAction *action)
                                       { }];
        [alertController addAction:cancelAction];
        
        UIAlertAction *otherAction = [UIAlertAction actionWithTitle:@"允许" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
                                      {
...
{
    NSString *commonString = @"00:00";
    if (aSecond) {
        NSInteger hour = aSecond/3600;
        NSInteger minute = (aSecond%3600)/60;
        NSInteger second = (aSecond%3600)%60;
        
        if (hour > 0) {
            commonString = [NSString stringWithFormat:@"%02tu:%02tu:%02tu", hour, minute, second];
        } else {
            commonString = [NSString stringWithFormat:@"%02tu:%02tu", minute, second];
        }
    }
    return commonString;
}
...
+ (void)openSystemSetting
{
    ...
    if ([[UIApplication sharedApplication] canOpenURL:url]) {
        //打开该应用的通知设置界面
        //        NSURL *url = [NSURL URLWithString:@"prefs:root=NOTIFICATIONS_ID&path=com.msb.LLbox"];
